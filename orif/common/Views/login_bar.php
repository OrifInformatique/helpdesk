...
//$CI->config->load('user/MY_user_config');
    ...
    /**le fichier user/MY_user_lang.php se situe dans la version 4 dans le dossier /system */
      ...
      <div class="nav flex-column">
        <?php if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] == true) { ?>
          ...
          <?php if ($_SESSION['user_access'] >= $config->item('access_lvl_admin')) { ?>
              <a href="<?php echo base_url("user/admin/list_user"); ?>" ><?php echo lang('MY_user_lang.btn_admin'); ?></a><br />
          <?php } ?>
          <!-- END OF ADMIN ACCESS -->

          <!-- Logged in, display a "change password" button -->
          <a href="<?php echo base_url("user/auth/change_password"); ?>" ><?php echo lang('MY_user_lang.btn_change_my_password'); ?></a>
          <!-- and a "logout" button -->
          <a href="<?php echo base_url("user/auth/logout"); ?>" ><?php echo lang('MY_user_lang.btn_logout'); ?></a><br />

        <?php } else { ?>
          <!-- Not logged in, display a "login" button -->
          <a href="<?php echo base_url("user/auth/login"); ?>" ><?php echo lang('MY_user_lang.btn_login'); ?></a>
        <?php } ?>
      </div>
    </div>
  </div>
